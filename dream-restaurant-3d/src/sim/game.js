...
export function buildFloorPlan(seed = 1, rooms = {}) {
  ...
  function buildFloorTables(floorIdx, rng2) {
    ...
    if (floorIdx === 0) {
      ...
      let n = 0;
      // ── 北側の帯（z ≈ -4.0）：2 人掛け ──
      mk('t' + (++n), -3.5, -4.05, 2, 'table');
      mk('t' + (++n), -1.5, -4.05, 2, 'table');
      // ── 西側の列：6 人掛け・4 人掛け ──
      mk('t' + (++n), -3.5, -2.62, 6, 'table');
      mk('t' + (++n), -3.5, -0.32, 4, 'table');
      mk('t' + (++n), -3.5, 1.88, 4, 'table');
      // ── 中央の列：4 人掛け ──
      mk('t' + (++n), -0.9, -0.32, 4, 'table');
      mk('t' + (++n), -0.9, 1.88, 4, 'table');
      // ── 南側の帯（z ≈ 3.4）：2 人掛け ──
      const pet1 = mk('t' + (++n), -3.6, 3.40, 2, 'table');
      const pet2 = mk('t' + (++n), -1.6, 3.40, 2, 'table');
      // ── 東側の壁ぎわ：1 人掛け（ひとり様席）──
      mk('t' + (++n), 6.0, -2.45, 1, 'solo');
      mk('t' + (++n), 5.95, -0.10, 1, 'solo');
      // ── 座敷（tatami）：4 人掛けの矮桌 ×4 ──
      const pet3 = mk('t' + (++n), 3.3, 1.70, 4, 'chabudai');
      const pet4 = mk('t' + (++n), 5.1, 1.70, 4, 'chabudai');
      mk('t' + (++n), 3.3, 3.30, 4, 'chabudai');
      mk('t' + (++n), 5.1, 3.30, 4, 'chabudai');
      ...
      let n = floorIdx * 100;
      // 北側の帯：2 人掛け
      mk('t' + (++n), -3.0, -4.05, 2, 'table');
      mk('t' + (++n), -0.8, -4.05, 2, 'table');
      // 西側：6 人掛け → 中央：4 人掛け ×2 列
      mk('t' + (++n), -3.4, -2.62, 6, 'table');
      mk('t' + (++n), -3.4, -0.28, 4, 'table');
      mk('t' + (++n), -0.6, -0.28, 4, 'table');
      mk('t' + (++n), 2.2, -0.28, 4, 'table');
      mk('t' + (++n), -3.4, 1.80, 4, 'table');
      mk('t' + (++n), -0.6, 1.80, 4, 'table');
      mk('t' + (++n), 2.2, 1.80, 4, 'table');
      // 南側の帯：2 人掛け＋6 人掛け
      mk('t' + (++n), -3.2, 3.60, 2, 'table');
      mk('t' + (++n), -1.0, 3.60, 2, 'table');
      mk('t' + (++n), 2.6, 3.90, 6, 'table');
      // 東側の壁ぎわ：1 人掛け
      mk('t' + (++n), 5.9, -1.45, 1, 'solo');
      mk('t' + (++n), 5.9, 1.20, 1, 'solo');
    }
    return tables;
  }
